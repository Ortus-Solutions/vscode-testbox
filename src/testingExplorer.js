...
/**
 * The glob pattern used to find test files in the workspace.
 */
const testFileGlob = "**/*{Spec,Specs,Test,Tests}.{cfc,bx}";
...
/**
 * Retrieves the Testbox settings from the workspace configuration.
 *
 * @returns {vscode.WorkspaceConfiguration} The Testbox settings.
 */
function getSettings(){
	return vscode.workspace.getConfiguration( "testbox" );
}
...
function createTestingViewController() {
	const controller = vscode.tests.createTestController( "TestBoxTestController", "TestBox Tests" );
...
async function discoverTests( controller ) {
	...
	const runnerUrl = getSettings().get( "runnerUrl" );
	// console.log(runnerUrl);
	const excludedPaths = getSettings().get( "excludedPaths" );
...
async function runTestsViaURL( request, token, controller ) {
	...
	const runnerUrl = getSettings().get( "runnerUrl", null );
	...
	const filteredTests = request.include.filter( test => {
		const testMeta = testData.get( test );
...
/**
 * Converts a given file path to a test name by removing the .cfc|bx extension
...
function convertToDottedPackageName( filePath ) {
	// Remove the .cfc|bx extension (case-insensitive)
	const withoutExtension = filePath.replace( /\.(cfc|bx)$/i, "" );
